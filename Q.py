...
def calculate_realized_volatility(self, 
    ...
    if use_weighted_returns:
        ...
        # For log returns: use weighted portfolio value (not fee_price directly)
        if self.use_log_returns:
            # Calculate weighted portfolio values using normalized non-cash weights
            portfolio_values = self.calculate_weighted_portfolio_value(df)
            returns = self.calculate_log_returns(portfolio_values, lag=self.n)
        else:
            # For ordinary returns: use fee_price or calculated value
