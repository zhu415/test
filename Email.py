...
“””
Find the most recent breach for each index across all breach types.
“””
index_breaches = {}

```
# Process all breach types
for breach_type, breaches in results.items():
    for breach in breaches:
        index_name = breach['index']
        comp_date = breach['comp_date']
        
        # Initialize or update with most recent breach
        if index_name not in index_breaches:
            ...
            # Compare dates and keep most recent
            if comp_date > index_breaches[index_name]['comp_date']:
                index_breaches[index_name] = {
                    'breach': breach,
                    'breach_type': breach_type,
                    'comp_date': comp_date
...
```
for index_name, breach_info in breach_summary.items():
    breach = breach_info['breach']
    ...
    record = {
        ...
        'Breach Type': format_breach_type(breach_type),
        'Breach Details': format_breach_details(breach, breach_type),
        **constituent_info,
        **borrow_shift_info
    }
...
```
    <h3>Breach Details (Most Recent per Index)</h3>
    {table_html}
    
    <div style="margin-top: 30px; padding: 10px; background-color: #f8f9fa; border-left: 4px solid #3498db;">
        <p><small>This is an automated alert. For questions, please contact the Risk Management team.</small></p>
    </div>
</body>
</html>
""".format(
