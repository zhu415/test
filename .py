...
def format_time(time_value):
    ...
    try:
        # Handle if it's already a timedelta
        if isinstance(time_value, timedelta):
            total_seconds = int(time_value.total_seconds())
        # Handle if it's a time string like "3:55:24"
        ...
        # Handle if it's a datetime.time object
...
def create_final_dataframe(extracted_data, field_names):
    ...
    # Create summary dataframe grouped by field_name
    final_data = []
    
    for field_name in field_names:
        field_data = cleaned_data[cleaned_data['field_name'] == field_name]
        
        if not field_data.empty:
            # Take the first row (or you could aggregate if multiple files)
            row = field_data.iloc[0]
            
            gnu1 = row.get('gnu1_value', 0)
            gnu2 = row.get('gnu2_value', 0)
            gnu_diff = row.get('gnu_diff_value', 0)
            
            # Calculate relative diff in percentage
            relative_diff = (gnu_diff / gnu1 * 100) if gnu1 != 0 else 0
            diff_div_gnu1 = (gnu_diff / gnu1) if gnu1 != 0 else 0
    ...
    for col in ['GNU 1 ($)', 'GNU 2 ($)', 'GNU diff ($)']:
        if col in formatted_df.columns:
            formatted_df[col] = formatted_df[col].apply(format_number_k)
    
    # Keep percentages and ratios as numbers (could format differently if needed)
    formatted_df['Relative diff (%)'] = formatted_df['Relative diff (%)'].apply(lambda x: f"{x:.2f}%" if pd.notna(x) else '')
    formatted_df['diff / GNU 1'] = formatted_df['diff / GNU 1'].apply(lambda x: f"{x:.4f}" if pd.notna(x) else '')
    
    # Format time columns
    for col in ['Calc time prod', 'Calc time qa', 'Max calc time prod', 'Max calc time qa']:
        if col in formatted_df.columns:
            formatted_df[col] = formatted_df[col].apply(format_time)
...
if __name__ == "__main__":
    ...
    if not extracted_data.empty:
        ...
        print("="*80)
        numeric_cols = ['GNU 1 ($)', 'GNU 2 ($)', 'GNU diff ($)', 'Relative diff (%)']
        for col in numeric_cols:
            if col in raw_df.columns and raw_df[col].dtype in ['float64', 'int64']:
                print(f"\n{col}:")
                print(f"  Mean: {raw_df[col].mean():.2f}")
                print(f"  Std:  {raw_df[col].std():.2f}")
                print(f"  Min:  {raw_df[col].min():.2f}")
                print(f"  Max:  {raw_df[col].max():.2f}")
    else:
