...
class RealizedVolatilityCalculator:
    ...
    def calculate_realized_volatility(self, 
        ...
        log_returns = self.calculate_log_returns(portfolio_values, lag=self.n)
        
        # Determine T0
        if T0_date:
            T0_idx = portfolio_values.index.get_loc(pd.Timestamp(T0_date))
        else:
            T0_idx = min(self.N, len(portfolio_values) - 1)
        
        # Calculate initial variances at T0
        initial_var_s = self.calculate_initial_variance(log_returns, T0_idx, is_short_term=True)
        initial_var_l = self.calculate_initial_variance(log_returns, T0_idx, is_short_term=False)
        
        # Calculate EWMA variances starting from T0
        returns_from_T0 = log_returns.iloc[T0_idx:]
    ...
    def calculate_with_leverage(self,
        ...
        vol_df['applied_leverage'].fillna(1.0, inplace=True)
...
# volatility_results = calc.calculate_realized_volatility(
#     df,
#     T0_date='2024-03-01'  # Optional: specify start date
# )

# Step 3: View results
# print(volatility_results.head())
# print(f"Average realized volatility: {volatility_results['realized_vol'].mean():.4f}")

# Step 4: Calculate with leverage (optional)
# volatility_with_leverage = calc.calculate_with_leverage(
#     df,
#     target_volatility=0.10,  # 10% target volatility
#     max_leverage=2.0,        # Maximum 2x leverage
#     lag_days=2,              # 2-day implementation lag
#     T0_date='2024-03-01'
# )
