...
def calculate_weighted_portfolio_returns(self, df: pd.DataFrame) -> pd.Series:
    ...
    sum(weight_on_previous_date * return_of_asset_from_previous_to_t)
    where return_of_asset = (price_t - price_{t-1}) / price_{t-1}
    ...
    # Process each date (starting from second date since we need previous weights)
            ...
            for _, prev_row in prev_data.iterrows():
                ...
                if len(curr_symbol_data) > 0:
                    ...
                    if prev_price > 0:  # Avoid division by zero
                        # Calculate return: (price_t - price_{t-1}) / price_{t-1}
...
def calculate_weighted_individual_log_returns(self, df: pd.DataFrame) -> pd.Series:
    """
    Calculate weighted sum of individual asset log returns.
    
    Return on date t = sum(normalized_weight_{t-1} * log(price_t / price_{t-1}))
...
def calculate_realized_volatility(self, 
    ...
    if self.use_log_returns:
        ...
        if self.use_individual_log_returns:
            ...
            returns = self.calculate_weighted_individual_log_returns(df)
